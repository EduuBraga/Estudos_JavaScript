...
let car = {
  ...
  run: () => {
    car.isRunning = true
    console.log(`O ${car.name} está em movimento`)
  },
  stop: () => {
    car.isRunning = false
    console.log(`O ${car.name} está parado`)
  },
  getColorsMessage: () => {
    console.log(`O ${car.name} está disponível nas cores ${car.colors.join(', ').replace(', Prata', ' e Prata.')}`)
  }
}
