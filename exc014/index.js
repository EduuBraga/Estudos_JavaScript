...
console.log(convertToString(244))
...
const quantityCharacters = (string) => {
  string = String(string)
  return string.length
}

console.log(quantityCharacters('eduardo'))
...
const transformLowerCase = string => string.toLowerCase()

console.log(transformLowerCase("CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO"))
...
const characterOfTheString = (charac, string) => string.indexOf(charac)

console.log(characterOfTheString('@', 'eduardo.braga467@gmail.com'))
...
const existsInTheArray = (item, array) => array.includes(item)

console.log(existsInTheArray('eduardo', ['carlos', 'eduardo', 'ponciano', 'braga']))
...
const concatenationFromArrays = (firstArray, secondArray) => firstArray.concat(secondArray)

console.log(concatenationFromArrays(['carlos', 'eduardo'], ['Ponciano', 'Braga']))
...
console.log(removeLastItem(['beterraba', 'abacaxi', 'mamão']))
...
const functionHandleCallback = callback => {
  callback()
}

const showName = () => console.log('eduardo')

functionHandleCallback(showName)
...
const showAge = (value, callback) => callback(value)

const threefoldNumber = number => number * 3

console.log(showAge(33, threefoldNumber))
...
const qunatityPersonLikes = (array) => {
  let msg = null

  if (array.length === 0) {
    msg = "Ninguém curtiu isso"
  }
  else if (array.length === 1) {
    msg = `Apenas ${array[0]} curtiu isso`
  }
  else if (array.length === 2) {
    msg = `${array[0]} e ${array[1]} curtiram isso`
  }
  else if (array.length === 3) {
    msg = `${array[0]}, ${array[1]} e ${array[2]} curtiram isso`
  }
  else if (array.length >= 4) {
    let restante = array.length - 2

    msg = `${array[0]}, ${array[2]} e mais ${restante} pessoas curtiram isso`
  }

  return msg
}

console.log(qunatityPersonLikes(['eduardo', 'carlos', 'vivian', 'barão']))
