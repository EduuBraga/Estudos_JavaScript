...
const getUserGitHub = async () => {
  const response = await fetch('https://api.github.com/users/EduuBraga')
...
const divisible2or3 = numbers.filter(num => num % 2 === 0 || num % 3 === 0)
...
const name1 = ['e', 'du', 'ar', 'do']
const name2 = ['vi', 'tó', 'ri', 'a']
// const PnameLanguage = name1.map(syllable => `p${syllable}`).join('')
// const PnameLanguage = name1
//   .reduce((acc, syllable) => `${acc}P${syllable}`, '')

const getNamePLanguage = name =>
  name.reduce((acc, syllable) => `${acc}P${syllable}`, '')

console.log(getNamePLanguage(name1))
console.log(getNamePLanguage(name2))
...
const secondName = 'eduardo'

const splittedNames = (name, separator) => {
  const splittedNames = name.split(separator)

  splittedNames.forEach((letter, index) =>
    console.log(`${letter.toUpperCase()} é a ${++index}ª letra do meu nome`))
}

splittedNames(secondName, '')
splittedNames(firstName, '')

// const splittedName = firstName.split('')
// splittedName.forEach((letter, index) =>
//   console.log(`${letter.toUpperCase()} é a ${++index}ª letra do meu nome`))

// for (let i = 0; i < firstName.length; i++) {
//   const letter = firstName.split('')
//   console.log(`${letter[i].toUpperCase()} é a ${i + 1}ª letra do meu nome`)
// }
...
const occurofValues = (array, value) =>
  array.reduce((acc, item) => item === value ? acc + 1 : acc, 0)

// const occurofValues = (array, value) => {
//   const filteredOccurof = array.filter(item => item === value)

//   return filteredOccurof.length
// }
...
console.log(occurofValues(scores, 100))
...
const filter = (arrayParameter, func) => arrayParameter
  .reduce((acc, item, index, array) => {
    func(item, index, array) ? acc.push(item) : acc
