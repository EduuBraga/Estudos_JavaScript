...
const resultHandleSubmit = document.querySelector('.result-handleSubmit')
...
const showResult = result => {
  resultHandleSubmit.innerText = result
  clearInput()
}
...
const handleSubmit = event => {
  ...
  const RegexInputValid = /^[a-zA-Z0-9]{7,11}$/
  const isAValidInput = RegexInputValid.test(input.value)

  if(isAValidInput){
    showResult('O valor inserido no input é válido =)')
    return
  }
  
  showResult('Valor inválido =(')
}
...
const result = pattern.test(paragraph.innerText)
console.log(result)
