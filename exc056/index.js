...
const getMessageCalculator = (num1, num2, operator, operation) =>
  `Resultado da operação: ${num1} ${operator} ${num2} = ${operation}.`

const calculator = operator => (num1, num2) => {
  const operations = {
    '+': getMessageCalculator(num1, num2, operator, num1 + num2),
    '-': getMessageCalculator(num1, num2, operator, num1 - num2),
    '*': getMessageCalculator(num1, num2, operator, num1 * num2),
    '/': getMessageCalculator(num1, num2, operator, num1 / num2),
    '%': getMessageCalculator(num1, num2, operator, num1 % num2)
...
const sudeste = ['São Paulo', 'Rio de Janeiro', 'Espírito Santo', 'Minas Gerais']

let brazil = sul.concat(sudeste)

brazil.unshift('Acre', 'Amapá', 'Amazonas')

console.log(brazil.shift())

const newSul = brazil.filter(state => sul.includes(state))
...
const newSudeste = brazil.splice(5, 4)

brazil = brazil.concat(nordeste)

const newBrasil = brazil.map((state, index) => ({ id: index, estado: state }))

const statesMore7Word = () => brazil.every(state => state.length >= 7)
  ? 'Sim, todos os estados tem mais de 7 letras.'
  : "Nem todos os estados tem mais de 7 letras."

console.log(statesMore7Word())
...
const cearaIncluded = () => brazil.some(state => state === 'Ceará')
  ? "Ceará está incluído."
  : "Ceará não foi incluído =/"

const refactorNewBrazil = newBrasil.reduce((acc, { id, estado }) => {
  acc.push({
    id: ++id,
...
const filteredRefactorNewBrazil = refactorNewBrazil.filter(state => state.id % 2 === 0)

console.log(filteredRefactorNewBrazil, newNewBrazil)
