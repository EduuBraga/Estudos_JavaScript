...
myNameArray[0] = myNameArray[0].toUpperCase()
...
const obj = {
  a: 1,
  b: 2,
  verifyNumnber: number => number > 50 ? obj.c = 3 : obj.d = 4
}

obj.verifyNumnber(randomNumber)
console.log(obj)
...
myForEach(oddNumbers, item => {
  accumulator += item;
})

console.log(accumulator)

myForEach(oddNumbers, (item, index, array) => {
  console.log(`"${item}" é o ${index}º item do array [${array}]`)
})
...
const changeImgCarousel = action => {
  toggleCurrentImg(action)
...
buttonPrev.addEventListener('click', _ => changeImgCarousel('prev'))