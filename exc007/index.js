...
for (let i = 0; i < fruits.length; i++) {
  if (fruits[i] === 'pera') {
    console.log('A string "Pera" no array')
  } else if (fruits[i] === 'abacaxi') {
    console.log('A string "Abacaxi" existe no array')
  } else {
    console.log('Não existe nem abacaxi e nem pera no array')
  }
}
...
let arrayOfNumbers11a90 = []

for (let i = 0; i < numbers.length; i++) {
  if (numbers[i] >= 11 && numbers[i] <= 90) {
    let itemFiltered = numbers[i]
    arrayOfNumbers11a90.push(itemFiltered)
  }
}

console.log(arrayOfNumbers11a90)
/*
  05
...
for (let i = 0; i < crazyArray.length; i++) {
  if (typeof crazyArray[i] === 'boolean') {
    itemsIsBoolean.push(crazyArray[i])
  }
  if (typeof crazyArray[i] === 'string') {
    itemsIsStrings.push(crazyArray[i])
  }
  if (typeof crazyArray[i] === 'number') {
    itemsIsNumbers.push(crazyArray[i])
  }
}

console.log(`O crazyArray tem ${itemsIsBoolean.length} booleans, ${itemsIsNumbers.length} números e ${itemsIsStrings.length} strings`)
...
console.log(`Numeros ímpares: ${impares.join(', ').replace(', 3', ' e 3')}. 
Números pares: ${pares.join(', ').replace(', 5', ' e 5')}.`)