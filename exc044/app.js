...
peopleCopy.sort((person1, person2) => person1.score - person2.score)
...
const animals3lyrics = animals.filter(animal => animal.length === 3)
...
const qtylyricsAnimals = animals.map(animals => animals.length)
...
const friendsNearMe = friends.map(friend => friend)
...
const oddNumbers = numbers.filter(number => number % 2)
  .reduce((some, numberCurrent) => some + numberCurrent, 0)
...
const totalPopulation = data.filter(country => country.country !== 'China')
  .reduce((populationTotal, countryCurrent) =>
    populationTotal + countryCurrent.population
    , 0)
