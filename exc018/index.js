...
best2019Movies.forEach( item => theBest += ` 
  - ${item.title} dirigido por ${item.directedBy}`)

console.log(`Segundo o site Omelete, os melhores filmes de 2019 são: ${theBest}`)
...
const youtubeUser = {
  ...
  expor(){
    let videos = ''
    youtubeUser.videos.recentVideos.forEach(item => videos += `
  - ${item.title}`)

    console.log(`Vídeos recentes de Roger Melo: ${videos}`)
  }
}
