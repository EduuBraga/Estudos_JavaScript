...
elementsInsideDiv.forEach(element => {
  element.addEventListener('click', event => {
    ...
    h2.innerText = `Clicou no ${tagNameLowercase}, filho da div.`
  })
})
...
egg.addEventListener('mousemove', event => {
  ...
  element.innerText = `Eixo X: ${event.offsetX} | Eixo Y: ${event.offsetY}`
})
...
const shouldFrontEnd = people.some(({ profession }) => profession === 'Front-end developer')
