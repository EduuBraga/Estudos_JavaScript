...
const namesAZ = names.map(item => item).sort()

console.log(names, namesAZ)
...
const charactersOrder = characters.map(item => item)
...
const numbersOrder = numbers.map(item => item)
...
const More50 = randomNumbers.find(item => item > 50)
...
const peopleZA = people.map(item => item)
  .sort().reverse()

console.log(peopleZA)
...
const stringIngredients = ingredients.reduce((acc, item, index, array) => {
  ...
  if (item.lastIndexOf('a') === item.length - 1) {
...
let templateHTML = topBrazilmovies.map(movie => `<li>${movie.title}</li>`)

templateHTML.forEach(item => listGroup.innerHTML += item)
